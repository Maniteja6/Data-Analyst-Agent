...
from backend.domain.intelligence.entities.task_node import TaskNode, TaskStatus, AgentRole
...
class ExecutionPlan(AggregateRoot):
    ...
    def record_task_complete(self, task_id: str, duration_ms: int) -> None:
        ...
        self._record_event(AgentResultReady(
    ...
    def get_ready_tasks(self, completed_ids: set[str]) -> list[TaskNode]:
        ...
        ready = [
            ...
            if t.is_ready and t.can_run(completed_ids)
        ]
    ...
    @classmethod
    def create_default(cls, plan_id: str, session_id: str, dataset_id: str,
                       has_datetime: bool = False, has_enough_numerics: bool = False) -> "ExecutionPlan":
        ...
        tasks = [
            TaskNode(id=new_uuid(), plan_id=plan_id, agent=AgentRole.SCHEMA,         depends_on=[],              priority=1),
            TaskNode(id=new_uuid(), plan_id=plan_id, agent=AgentRole.PROFILING,       depends_on=[],              priority=1),
        ]
        schema_id   = tasks[0].id
        profiling_id = tasks[1].id

        cleaning = TaskNode(id=new_uuid(), plan_id=plan_id, agent=AgentRole.CLEANING,
                            depends_on=[schema_id, profiling_id], priority=2)
        tasks.append(cleaning)

        sql_id = new_uuid()
        tasks.append(TaskNode(id=sql_id, plan_id=plan_id, agent=AgentRole.SQL,
                              depends_on=[cleaning.id], priority=3))
        rag_id = new_uuid()
        tasks.append(TaskNode(id=rag_id, plan_id=plan_id, agent=AgentRole.RAG,
                              depends_on=[cleaning.id], priority=3))
        ...
        if has_datetime:
            fc_id = new_uuid()
            tasks.append(TaskNode(id=fc_id, plan_id=plan_id, agent=AgentRole.FORECAST,
                                  depends_on=[cleaning.id], priority=3))
            analysis_deps.append(fc_id)

        if has_enough_numerics:
            ml_id = new_uuid()
            tasks.append(TaskNode(id=ml_id, plan_id=plan_id, agent=AgentRole.ML,
                                  depends_on=[cleaning.id], priority=3))
            analysis_deps.append(ml_id)

        insight_id = new_uuid()
        tasks.append(TaskNode(id=insight_id, plan_id=plan_id, agent=AgentRole.INSIGHT,
                              depends_on=analysis_deps, priority=4))
        critic_id = new_uuid()
        tasks.append(TaskNode(id=critic_id, plan_id=plan_id, agent=AgentRole.CRITIC,
                              depends_on=[insight_id], priority=5))
        rec_id = new_uuid()
        tasks.append(TaskNode(id=rec_id, plan_id=plan_id, agent=AgentRole.RECOMMENDATION,
                              depends_on=[critic_id], priority=6))
        report_id = new_uuid()
        tasks.append(TaskNode(id=report_id, plan_id=plan_id, agent=AgentRole.REPORT,
                              depends_on=[rec_id], priority=7))
