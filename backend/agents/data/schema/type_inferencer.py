...
_CURRENCY_KEYWORDS= frozenset({"price", "revenue", "cost", "amount", "salary", "fee", "total",
                                "subtotal", "tax", "income", "spend", "budget", "payment",
                                "charge", "earnings", "profit", "margin", "sales", "usd",
                                "eur", "gbp", "jpy", "$", "£", "€"})
_PCT_KEYWORDS     = frozenset({"%", "pct", "percent", "rate", "ratio", "share", "fraction",
                                "completion", "discount", "margin"})
_DATE_KEYWORDS    = frozenset({"date", "day", "month", "year", "dt", "timestamp", "time",
                                "created_at", "updated_at", "deleted_at", "start", "end",
                                "opened", "closed", "published", "birth", "hired"})
_EMAIL_KEYWORDS   = frozenset({"email", "e_mail", "mail", "contact"})
_PHONE_KEYWORDS   = frozenset({"phone", "tel", "mobile", "cell", "fax", "contact_no"})
_COUNT_KEYWORDS   = frozenset({"count", "qty", "quantity", "num_", "n_", "items", "units",
                                "orders", "transactions", "visits", "clicks", "views",
...
def infer_all_columns(df, emit_progress=None) -> list[TypeInference]:
...
def infer_column(df, col: str, is_polars: bool | None = None) -> TypeInference:
...
def _infer_polars(df, col: str) -> TypeInference:
    import polars as pl
    ...
    if dtype in (pl.Date,):
        return TypeInference(col, dtype_str, "date",     1.0, False, sample_vals, null_rate, unique_count)

    if dtype in (pl.Datetime, pl.Time, pl.Duration):
        return TypeInference(col, dtype_str, "datetime", 1.0, False, sample_vals, null_rate, unique_count)

    if dtype == pl.Boolean:
        return TypeInference(col, dtype_str, "boolean",  1.0, False, sample_vals, null_rate, unique_count)

    # ── Keyword-driven name matching (high confidence) ────────────────────

    if any(kw in name for kw in _EMAIL_KEYWORDS) and "@" in " ".join(sample_vals):
        return TypeInference(col, dtype_str, "email",    0.95, False, sample_vals, null_rate, unique_count)

    if any(kw in name for kw in _PHONE_KEYWORDS):
        return TypeInference(col, dtype_str, "phone",    0.85, False, sample_vals, null_rate, unique_count)

    if any(kw in name for kw in _PCT_KEYWORDS):
        return TypeInference(col, dtype_str, "percentage", 0.85, False, sample_vals, null_rate, unique_count)

    if any(kw in name for kw in _CURRENCY_KEYWORDS):
        return TypeInference(col, dtype_str, "currency", 0.90, False, sample_vals, null_rate, unique_count)
    ...
    if dtype == pl.Utf8:
        ...
        if any(kw in name for kw in _DATE_KEYWORDS):
            return TypeInference(col, dtype_str, "datetime",        0.70, False, sample_vals, null_rate, unique_count)

        # Check identifier patterns
        if any(kw in name for kw in _ID_KEYWORDS):
            return TypeInference(col, dtype_str, "identifier",      0.75, False, sample_vals, null_rate, unique_count)
        ...
        if cardinality_ratio <= _CATEGORICAL_CARDINALITY_RATIO:
            return TypeInference(col, dtype_str, "categorical",     0.80, False, sample_vals, null_rate, unique_count)

        # Ambiguous — forward to LLM classifier
        return TypeInference(col, dtype_str, "free_text",           0.50, True,  sample_vals, null_rate, unique_count)

    # ── Unknown / struct / list types ─────────────────────────────────────
    return TypeInference(col, dtype_str, "unknown", 0.0, True, sample_vals, null_rate, unique_count)


def _infer_pandas(df, col: str) -> TypeInference:
    import numpy as np
    ...
    if dtype == object or "str" in str(dtype):
        cardinality_ratio = unique_count / max(total_rows - null_count, 1)
        if any(kw in name for kw in _DATE_KEYWORDS):
            return TypeInference(col, dtype_str, "datetime", 0.65, False, sample_vals, null_rate, unique_count)
        if cardinality_ratio <= _CATEGORICAL_CARDINALITY_RATIO:
            return TypeInference(col, dtype_str, "categorical", 0.80, False, sample_vals, null_rate, unique_count)
        return TypeInference(col, dtype_str, "free_text", 0.50, True, sample_vals, null_rate, unique_count)
