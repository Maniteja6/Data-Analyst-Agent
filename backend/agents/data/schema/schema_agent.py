...
class SchemaAgent(BaseAgent):
    ...
    async def _execute(self, context: AgentContext, **kwargs: Any) -> dict:
        ...
        await context.push_progress(
            6, f"Inferring types for {col_count} columns…", step="schema"
        )
        ...
        inferences: list[TypeInference] = await asyncio.get_event_loop().run_in_executor(
            None, infer_all_columns, df, None   # emit_progress=None here; we do it below
        )
        ...
        columns = [
            {
                ...
                "is_primary_key": inf.semantic_type == "identifier" and inf.unique_count == row_count,
            }
            for inf in inferences
        ]
