...
from backend.domain.workspace.exceptions import ConversationNotFoundException
...
class SendMessageUseCase:
    ...
    def __init__(self, conversation_repo, dataset_repo, cache, llm_service) -> None:
    ...
    async def execute(self, cmd: SendMessageCommand) -> dict:
        ...
        if conversation is None:
            raise ConversationNotFoundException(cmd.conversation_id)
...
def _build_schema_summary(schema_json: dict | None) -> str:
    ...
    for c in cols[:30]:   # cap at 30 columns to keep context short
        lines.append(
            ...
            + (f" [{c.get('null_rate', 0.0)*100:.0f}% null]" if c.get("null_rate", 0) > 0.01 else "")
        )
    return "\n".join(lines)
