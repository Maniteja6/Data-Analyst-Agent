...
class TestAnalysisPipeline:
    @pytest.mark.asyncio
    async def test_profiler_then_cleaner_produces_report(self, sample_df) -> None:
    ...
    @pytest.mark.asyncio
    async def test_profiler_then_anomaly_detector(self, sample_df) -> None:
    ...
    @pytest.mark.asyncio
    async def test_correlation_engine_on_numeric_columns(self, sample_df) -> None:
        ...
        try:
            ...
            numeric_cols = [
                ...
                if sample_df[c].dtype in (pl.Float64, pl.Int64, pl.Float32, pl.Int32)
            ]
        except ImportError:
            import numpy as np

            numeric_cols = [
                c for c in sample_df.columns if np.issubdtype(sample_df[c].dtype, np.number)
            ]
...
class TestDuckDBManager:
    @pytest.mark.asyncio
    async def test_execute_sum_query(self, sample_df) -> None:
    ...
    @pytest.mark.asyncio
    async def test_execute_group_by(self, sample_df) -> None:
        from backend.analytics_engine.sql_engine.duckdb_manager import DuckDBManager
