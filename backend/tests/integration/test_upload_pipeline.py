...
class TestUploadPipeline:
    ...
    async def test_upload_returns_dataset_id_and_job_id(
        ...
        class _FakeRepo:
            saved = []
            async def get_by_checksum(self, c): return None
    ...
    @pytest.mark.asyncio
    async def test_duplicate_upload_raises(self, local_storage, null_job_service):
        from backend.application.use_cases.upload_dataset import UploadDatasetUseCase
        from backend.application.commands.upload_dataset_command import UploadDatasetCommand
        ...
        class _FakeRepo:
            async def get_by_checksum(self, c):
                return Dataset.create(
                    id="existing-id", project_id=None, original_name="old.csv",
        ...
        csv_data = b"col_a,col_b\n1,hello\n"
        cmd = UploadDatasetCommand(
            filename="dup.csv", file_obj=io.BytesIO(csv_data),
            size_bytes=len(csv_data), mime_type="text/csv",
        )
