...
class TestDataQualityScorer:
    ...
    def test_perfect_profile_scores_above_90(self) -> None:
        profile = self._make_profile()
        report = self.scorer.score(profile)
        assert report.overall_score >= 0.90
        assert report.grade in ("A", "B")

    def test_low_completeness_lowers_score(self) -> None:
        profile = self._make_profile(completeness_score=0.5)
        report = self.scorer.score(profile)
        assert report.overall_score < 0.80

    def test_high_null_rate_columns_reduce_validity(self) -> None:
        class _NullyCol:
            null_rate = 0.6
    ...
    def test_datetime_columns_boost_timeliness(self) -> None:
        class _DtCol:
            pass
