...
class TestDatasetAggregate:
    def test_create_emits_dataset_uploaded_event(self, fake_dataset) -> None:
        # Events are consumed in fixture; create a fresh one
        ds = Dataset.create(
            id="test-id",
            project_id=None,
            original_name="test.csv",
            storage_key="datasets/test-id/test.csv",
            size_bytes=100,
            mime_type="text/csv",
        )
    ...
    def test_initial_status_is_uploaded(self, fake_dataset) -> None:
        assert fake_dataset.status == DatasetStatus.UPLOADED

    def test_begin_profiling_transitions_status(self, fake_dataset) -> None:
        fake_dataset.begin_profiling()
        assert fake_dataset.status == DatasetStatus.PROFILING

    def test_invalid_transition_raises(self, fake_dataset) -> None:
        with pytest.raises(InvalidStatusTransitionError):
            fake_dataset.mark_ready(row_count=100, column_count=5)  # can't skip to READY

    def test_full_pipeline_state_machine(self) -> None:
        ds = Dataset.create(
            id="x",
            project_id=None,
            original_name="f.csv",
    ...
    def test_has_time_series_false_without_schema(self, fake_dataset) -> None:
        assert fake_dataset.has_time_series is False

    def test_has_schema_true_after_schema_set(self, fake_dataset) -> None:
