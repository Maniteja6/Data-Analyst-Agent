...
class TestDataProfiler:
    ...
    @pytest.mark.asyncio
    async def test_profile_returns_data_profile(self, sample_df):
    ...
    @pytest.mark.asyncio
    async def test_numeric_columns_have_stats(self, sample_df):
        from backend.analytics_engine.profiling.data_profiler import DataProfiler
    ...
    @pytest.mark.asyncio
    async def test_completeness_is_one_for_clean_data(self, sample_df):
        from backend.analytics_engine.profiling.data_profiler import DataProfiler
        profiler = DataProfiler()
        profile  = await profiler.profile(sample_df)
    ...
    @pytest.mark.asyncio
    async def test_duplicate_count_zero_for_unique_data(self, sample_df):
        from backend.analytics_engine.profiling.data_profiler import DataProfiler
        profiler = DataProfiler()
        profile  = await profiler.profile(sample_df)
    ...
    @pytest.mark.asyncio
    async def test_profile_handles_all_nulls_column(self):
