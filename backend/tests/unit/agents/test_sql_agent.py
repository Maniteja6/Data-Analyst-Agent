...
class TestSQLAgent:
    @pytest.mark.asyncio
    async def test_sql_agent_executes_generated_query(self, sample_df, mock_llm) -> None:
...
class TestQueryBuilder:
    def test_aggregate_builds_sum_query(self) -> None:
    ...
    def test_filter_rows_builds_where_clause(self) -> None:
        from backend.analytics_engine.sql_engine.query_builder import QueryBuilder
    ...
    def test_column_names_are_quoted(self) -> None:
        from backend.analytics_engine.sql_engine.query_builder import QueryBuilder
