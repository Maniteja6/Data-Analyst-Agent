...
class Settings(BaseSettings):
    ...
    # ── Application ───────────────────────────────────────────────────────
    app_env: str = Field("development", description="Runtime environment: development | staging | production")
    ...
    debug: bool = Field(False, description="Enable debug mode (verbose SQL, stack traces in API responses)")
    log_level: str = Field("INFO", description="Root log level: DEBUG | INFO | WARNING | ERROR")
    secret_key: str = Field("change-me-to-a-random-64-char-string-in-production", description="Used for signing internal tokens")

    # ── API server ────────────────────────────────────────────────────────
    api_host: str = Field("0.0.0.0", description="Bind address for uvicorn")
    ...
    enable_swagger: bool = Field(True, description="Expose /docs and /redoc (disable in production)")
    ...
    database_max_overflow: int = Field(20, ge=0, description="SQLAlchemy max connections above pool_size")
    ...
    # ── Celery ────────────────────────────────────────────────────────────
    celery_broker_url: str = Field("redis://localhost:6379/1", description="Celery message broker URL")
    ...
    celery_task_soft_time_limit: int = Field(300, ge=1, description="Celery soft task timeout in seconds")
    celery_task_time_limit: int = Field(600, ge=1, description="Celery hard task timeout in seconds")

    # ── AWS general ───────────────────────────────────────────────────────
    aws_region: str = Field("us-east-1", description="Primary AWS region")
    aws_access_key_id: str = Field("", description="Leave blank when using IRSA or instance profiles")
    aws_secret_access_key: str = Field("", description="Leave blank when using IRSA or instance profiles")

    # ── S3 / MinIO ────────────────────────────────────────────────────────
    s3_bucket_name: str = Field("datapilot-datasets-dev", description="Primary S3 bucket for uploaded datasets")
    s3_region: str = Field("us-east-1", description="S3 bucket region")
    s3_endpoint_url: str | None = Field(None, description="Override endpoint for MinIO (local dev). Leave blank for real AWS S3.")
    s3_presigned_url_ttl_seconds: int = Field(900, ge=60, description="Presigned download URL expiry — 15 minutes")
    s3_datasets_prefix: str = Field("datasets/", description="S3 key prefix for uploaded dataset files")
    s3_reports_prefix: str = Field("reports/", description="S3 key prefix for generated report files")
    s3_temp_prefix: str = Field("tmp/", description="S3 key prefix for temporary processing files — lifecycle rule expires after 7 days")

    # ── Kafka ─────────────────────────────────────────────────────────────
    kafka_bootstrap_servers: str = Field("localhost:9092", description="Comma-separated Kafka broker list")
    ...
    kafka_schema_registry_url: str = Field("http://localhost:8081", description="Confluent / Glue Schema Registry URL")
    kafka_consumer_group_id: str = Field("datapilot-analytics-engine", description="Consumer group for the analytics pipeline")
    kafka_auto_offset_reset: str = Field("earliest", description="earliest | latest")
    kafka_session_timeout_ms: int = Field(30000, description="Consumer session timeout in milliseconds")
    ...
    clickhouse_http_port: int = Field(8123, description="ClickHouse HTTP port (used by some clients)")
    ...
    qdrant_api_key: str = Field("", description="Qdrant API key (blank for local unauthenticated instance)")
    qdrant_collection_name: str = Field("datapilot_chunks", description="Collection storing dataset chunk embeddings")
    ...
    # ── Analytics engine ──────────────────────────────────────────────────
    anomaly_zscore_threshold: float = Field(3.0, description="Z-score threshold for outlier detection")
    ...
    profiling_sample_size: int = Field(100_000, description="Max rows sampled for histogram computation")
    profiling_top_n_values: int = Field(20, description="Top-N value frequencies stored per categorical column")

    # ── Agent configuration ───────────────────────────────────────────────
    agent_max_retries: int = Field(3, ge=1, description="Maximum agent execution attempts before failure")
    agent_retry_backoff_seconds: float = Field(2.0, description="Base exponential backoff delay between retries")
    agent_llm_cache_ttl_seconds: int = Field(86400, description="LLM response cache TTL — 24 hours")
    sql_agent_row_limit: int = Field(10_000, description="Maximum rows returned by the SQL agent per query")
    ...
    python_agent_memory_limit_mb: int = Field(512, description="Memory ceiling for sandboxed Python subprocess")
    ...
    rag_similarity_threshold: float = Field(0.72, description="Minimum cosine similarity score for a retrieved chunk to be used")
    critic_max_revision_rounds: int = Field(2, ge=1, description="Maximum Critic→Insight revision cycles")

    # ── Security ──────────────────────────────────────────────────────────
    pii_detection_enabled: bool = Field(True, description="Enable Presidio PII detection on user input and agent output")
    pii_score_threshold: float = Field(0.7, ge=0.0, le=1.0, description="Minimum Presidio confidence score to flag as PII")
    pii_redaction_placeholder: str = Field("<REDACTED>", description="String substituted for detected PII in outputs")
    injection_detection_enabled: bool = Field(True, description="Enable prompt injection detection on user messages")
    injection_score_threshold: float = Field(0.6, ge=0.0, le=1.0, description="Minimum injection score to block a request")
    rate_limit_requests_per_minute: int = Field(60, ge=1, description="API rate limit per client IP")
    rate_limit_upload_per_hour: int = Field(20, ge=1, description="Dataset upload rate limit per client IP")
    ...
    jwt_public_key: str = Field("", description="RS256 public key PEM (blank in local dev with no auth)")
    jwt_private_key: str = Field("", description="RS256 private key PEM (blank in local dev with no auth)")

    # ── Observability ─────────────────────────────────────────────────────
    otel_enabled: bool = Field(True, description="Enable OpenTelemetry tracing")
    otel_service_name: str = Field("datapilot-api")
    otel_exporter_otlp_endpoint: str = Field("http://localhost:4317", description="OTLP gRPC collector endpoint")
    ...
    def warn_insecure_defaults_in_production(self) -> "Settings":
        if self.app_env == "production":
            ...
            if self.enable_swagger:
                import warnings
                warnings.warn("enable_swagger=True in production exposes API internals", stacklevel=2)
        return self
