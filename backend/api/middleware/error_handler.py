...
from backend.shared.exceptions import DomainException, ValidationException, AgentException

logger = structlog.get_logger(__name__)

# Map DomainException error codes → HTTP status codes
...
def _domain_error_response(exc: DomainException) -> JSONResponse:
...
async def domain_exception_handler(request: Request, exc: DomainException) -> JSONResponse:
    logger.warning("domain_exception", code=getattr(exc, "code", "?"), path=request.url.path, error=str(exc))
...
async def request_validation_handler(request: Request, exc: RequestValidationError) -> JSONResponse:
    ...
    return JSONResponse(
        status_code=422,
        content={"error": "REQUEST_VALIDATION_ERROR", "message": "Invalid request data.", "details": errors},
    )
