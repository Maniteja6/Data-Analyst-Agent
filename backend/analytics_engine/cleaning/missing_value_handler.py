...
class MissingValueHandler:
    ...
    def handle(self, df, column_profiles: list) -> tuple:
        ...
        for cp in column_profiles:
            ...
            kind      = str(cp.kind.value if hasattr(cp.kind, "value") else cp.kind if hasattr(cp, "kind") else "unknown")
    ...
    def _drop_column(self, df, col: str, is_polars: bool, null_rate: float) -> tuple:
        if is_polars:
            df = df.drop(col)
        else:
        ...
        step = CleaningStep(
            ...
            column=col,
            rows_affected=0,
            description=f"Dropped column '{col}' ({null_rate*100:.0f}% nulls — exceeds threshold).",
        )
        logger.info("column_dropped_high_null", column=col, null_rate=null_rate)
        return df, step

    def _impute(self, df, col: str, kind: str, is_polars: bool) -> tuple:
        if kind == "numeric":
        ...
        null_count = int(getattr(df[col], "null_count", lambda: 0)() if is_polars
