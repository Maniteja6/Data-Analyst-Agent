...
class TypeCoercer:
    ...
    def coerce(self, df: DataFrameT, column_profiles: list) -> tuple:
        ...
        for cp in column_profiles:
            ...
            stype = str(
                ...
                if hasattr(getattr(cp, "semantic_type", None), "value")
    ...
    def _coerce_to_float(self, df: DataFrameT, col: str, is_polars: bool) -> tuple:
        try:
            if is_polars:
                df = df.with_columns(
                    df[col]
                ...
                df[col] = df[col].apply(
                    ...
                    errors="coerce" if hasattr(df[col], "astype") else None,
                )
            step = CleaningStep(
    ...
    def _coerce_to_datetime(self, df: DataFrameT, col: str, is_polars: bool) -> tuple:
        try:
            if is_polars:
                import polars as pl
