...
class CorrelationEngine:
    ...
    def compute(self, df: DataFrameT, numeric_columns: list[str]) -> list[CorrelationCoefficient]:
    ...
    def _compute_polars(self, df: DataFrameT, cols: list[str]) -> list[CorrelationCoefficient]:
        import polars as pl
        ...
        for i, col_a in enumerate(cols):
            for col_b in cols[i + 1 :]:
                try:
                    ...
                    logger.debug(
                        "pearson_corr_failed", column_a=col_a, column_b=col_b, error=str(exc)
                    )
                    continue
    ...
    def _compute_pandas(self, df: DataFrameT, cols: list[str]) -> list[CorrelationCoefficient]:
        import numpy as np
        ...
        for i, col_a in enumerate(cols):
            for col_b in cols[i + 1 :]:
                try:
                    ...
                    logger.debug(
                        "numpy_corr_failed", column_a=col_a, column_b=col_b, error=str(exc)
                    )
                    continue
        return sorted(results, key=lambda c: abs(c.value), reverse=True)
